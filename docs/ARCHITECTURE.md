...
- 删除线程或文件时由 Service 先将本地对象移动到同一卷的受控暂存名，再提交数据库；
  提交失败立即恢复。进程异常退出时，启动恢复根据数据库权威记录恢复未提交删除，
  或清理已提交删除及无元数据的受控对象。
...
- 应用启动不运行迁移，但会在已迁移 schema 上把遗留活动 run 标记为 cancelled、
  终结遗留工具调用并协调线程/文件暂存项，从而释放上次异常退出留下的会话锁。
